...
export function NavBar() {
  ...
  const { toggleProgOpen, navigate: storeNavigate } = useAppStore()
  const { curPoint, SAGA_META } = useProgress()
  const { lang, current: currentLang, options: LANG_OPTIONS, langOpen, setLang, toggleLangOpen, closeLangOpen } = useLang()
  ...
  return (
    ...
    >
      ...
      <button
        ...
        style={{ outline: 'none' }}
        aria-label={t('brand.ariaHome', 'Ir al inicio')}
      >
      ...
      <nav className="hidden md:flex items-center gap-1 flex-1 justify-center">
        {NAV_ITEMS.map(({ labelKey, section: sec, route }) => {
      ...
      <div className="flex items-center gap-2 shrink-0">
        ...
        <button
          ...
          title={t('progress.configureTitle', 'Configurar punto de progreso')}
        >
          {/* Colored dot */}
          <span
            style={{
        ...
        <div ref={langRef} style={{ position: 'relative' }}>
          <button
            ...
            onMouseEnter={e => (e.currentTarget.style.borderColor = 'var(--color-border-hover)')}
            onMouseLeave={e => (e.currentTarget.style.borderColor = 'var(--color-border)')}
            aria-label={t('lang.ariaSelect', 'Seleccionar idioma')}
          >
