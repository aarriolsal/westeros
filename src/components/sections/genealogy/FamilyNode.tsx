...
import type { FamilyFlowNode } from './buildGenealogyGraph'

export default function FamilyNode({ data: member }: NodeProps<FamilyFlowNode>) {
  const { houses: HOUSES, characters: CHARS } = useDataStore()
  ...
  const statusColor = character
    ? (character.status === 'alive' ? 'var(--color-alive)' : 'var(--color-dead)')
    : 'transparent'
