...
import FamilyNode from '@/components/sections/genealogy/FamilyNode'
import { buildGenealogyGraph, type FamilyFlowNode } from '@/components/sections/genealogy/buildGenealogyGraph'
import { TREE_HOUSE_IDS } from '@/config/tree'
import { translationNameSpace } from '@/config/lang'

const nodeTypes = { familyMember: FamilyNode }

export default function GenealogyPage() {
  const { t } = useTranslation(translationNameSpace.genealogy)
  const { genealogyHouse, setGenealogyHouse, setEncSel } = useAppStore()
  const { detail } = useEncyclopedia()
  const { familyTrees: FAMILY_TREES, houses: HOUSES } = useDataStore()

  function getHouseMeta(id: string) {
    return HOUSES.find(h => h.id === id)
  }

  const activeId = genealogyHouse || 'targaryen'
  const tree = FAMILY_TREES[activeId]
  const houseMeta = getHouseMeta(activeId)
  const accentColor = houseMeta?.accent ?? 'var(--color-gold)'

  const { nodes, edges } = useMemo(() => (tree ? buildGenealogyGraph(tree) : { nodes: [], edges: [] }), [tree])
        ...
        {tree && (
          ...
          >
            ...
            <p style={{
              ...
              {t('legend', 'Línea sólida: relación de padre/madre a hijo. Línea discontinua dorada: matrimonio. Los nombres resaltados con un punto de color son personajes con ficha propia — haz clic para verla.')}
            </p>
          </motion.div>
        )}
      </AnimatePresence>
